...
export default function UpdateCustomerForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const c: any = await getCustomerById(Number(id));
        setForm({ name: c.name || "", phone: c.phone || "", email: c.email || "", address: c.address || "" });
      } catch (err: any) {
        setErrorMessage(err?.response?.data?.message || "Failed to load customer.");
        setErrorOpen(true);
      }
    })();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // fetch updated customer and replace in cache
      const fresh: any = await getCustomerById(Number(id));
  ...
  return (
    <Stack alignItems="center" sx={{ mt: 4, px: isMobile ? 2 : 0 }}>
      <Paper sx={{ p: theme.spacing(3), maxWidth: "600px", width: "100%", boxShadow: 2, borderRadius: 2 }}>
        <Typography variant="h6" sx={{ mb: 3, textAlign: isMobile ? "center" : "left" }}>Update Customer</Typography>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", mt: 3, flexDirection: isMobile ? "column" : "row", gap: isMobile ? 2 : 0 }}>
          ...
          <Button variant="contained" fullWidth={isMobile} sx={{ backgroundColor: "var(--pallet-blue)" }} onClick={handleSubmit}>Update Customer</Button>
        </Box>
      </Paper>

      <UpdateConfirmationModal open={open} title="Success" content="Customer has been updated successfully!" handleClose={() => setOpen(false)} onSuccess={() => window.history.back()} />
