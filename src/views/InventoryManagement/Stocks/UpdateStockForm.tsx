...
import { getStocks, updateStock } from "../../../api/Stocks/stocksApi";
...
export default function UpdateStockForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setForm({ item_id: String(found.item_id ?? ""), quantity: String(found.quantity ?? "") });
      } catch (err: any) {
        console.error(err);
        setErrorMessage(err?.response?.data?.message || "Failed to load stock");
        setErrorOpen(true);
      }
    })();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateStock(Number(id), payload as any);
  ...
  return (
    <Stack alignItems="center" sx={{ mt: 4, px: isMobile ? 2 : 0 }}>
      <Paper sx={{ p: theme.spacing(3), maxWidth: "600px", width: "100%", boxShadow: 2, borderRadius: 2 }}>
        <Typography variant="h6" sx={{ mb: 3, textAlign: isMobile ? "center" : "left" }}>Update Stock</Typography>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", mt: 3, flexDirection: isMobile ? "column" : "row", gap: isMobile ? 2 : 0 }}>
          ...
          <Button variant="contained" fullWidth={isMobile} sx={{ backgroundColor: "var(--pallet-blue)" }} onClick={handleSubmit}>Update Stock</Button>
        </Box>
      </Paper>

      <UpdateConfirmationModal open={open} title="Success" content="Stock has been updated successfully!" handleClose={() => setOpen(false)} onSuccess={() => window.history.back()} />
