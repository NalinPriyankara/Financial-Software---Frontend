...
import { getSaleItemById, updateSaleItem } from "../../../api/SaleItems/saleItemsApi";
import { getSales } from "../../../api/Sales/salesApi";
import { getItems } from "../../../api/Items/itemsApi";
import type { SaleItemPayload } from "../../../api/SaleItems/saleItemsApi";

export default function UpdateSaleItemForm() {
  const { id } = useParams();
  const itemId = Number(id);
  const { data: si, isLoading } = useQuery<any, Error>({ queryKey: ["sale-item", itemId], queryFn: () => getSaleItemById(itemId), enabled: !!itemId });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "item_id") {
      ...
      setForm((p: any) => ({ ...p, [name]: value, price: priceVal }));
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateSaleItem(itemId, payload);

      // optimistic cache update
      try {
        const updated = { ...(si as any), ...payload, id: itemId };
        queryClient.setQueryData(["sale-items"], (old: any) => {
          if (!old) return [updated];
          if (Array.isArray(old)) return old.map((s: any) => (s.id === itemId ? updated : s));
          if (old?.data && Array.isArray(old.data)) return { ...old, data: old.data.map((s: any) => (s.id === itemId ? updated : s)) };
          return old;
        });
        queryClient.setQueryData(["sale-item", itemId], updated);
      } catch (e) {
        console.warn("Failed to update sale-items cache optimistically", e);
      }
      ...
      if (!data?.errors && !data?.message) { setErrorMessage("Failed to update sale item. Please try again."); setErrorOpen(true); }
    }
  };
  ...
  return (
    <Stack alignItems="center" sx={{ mt: 4, px: isMobile ? 2 : 0 }}>
      <Paper sx={{ p: theme.spacing(3), maxWidth: "600px", width: "100%", boxShadow: 2, borderRadius: 2 }}>
        <Typography variant="h6" sx={{ mb: 3, textAlign: isMobile ? "center" : "left" }}>Update Sale Item</Typography>
      ...
      <UpdateConfirmationModal open={open} title="Success" content="Sale item updated successfully!" handleClose={() => setOpen(false)} onSuccess={() => navigate(-1)} />
